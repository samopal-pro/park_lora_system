...
//	ETH_SPI->begin();
	initSS();
	resetSS();
	ETH_SPI->beginTransaction(SPI_ETHERNET_SETTINGS);
...
uint16_t W5100Class::write(uint16_t addr, const uint8_t *buf, uint16_t len)
{
	...
	if (chip == 51) {
		for (uint16_t i=0; i<len; i++) {
			setSS();
			ETH_SPI->transfer(0xF0);
			ETH_SPI->transfer(addr >> 8);
			ETH_SPI->transfer(addr & 0xFF);
			addr++;
...
uint16_t W5100Class::read(uint16_t addr, uint8_t *buf, uint16_t len)
{
	...
	if (chip == 51) {
		for (uint16_t i=0; i < len; i++) {
			setSS();
			#if 1
			ETH_SPI->transfer(0x0F);
			ETH_SPI->transfer(addr >> 8);
			ETH_SPI->transfer(addr & 0xFF);
			addr++;
			buf[i] = ETH_SPI->transfer(0);
			#else
			cmd[0] = 0x0F;
			cmd[1] = addr >> 8;
			cmd[2] = addr & 0xFF;
			cmd[3] = 0;
			ETH_SPI->transfer(cmd, 4); // TODO: why doesn't this work?
			buf[i] = cmd[3];
			addr++;
			#endif
			resetSS();
