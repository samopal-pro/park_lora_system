...
//   loraMutex = xSemaphoreCreateBinary();
//
    ...
    vTaskDelay(1000);
    xTaskCreateUniversal(taskLoRa, "lora", 4096, NULL, 3, NULL,CORE);   
...
#endif
      if( !isConnectStatus ){
         JNodes.nodesStatus[0]    = NLS_WIFI_OFF;
         wifi_error++;
      }
      else {
         JNodes.nodesStatus[0]    = NLS_WIFI_ON;
         wifi_error = 0;
      }
      if( isAP ){
...
//               WiFi_ScanNetworks();
               ...
               WiFi.mode(WIFI_AP);
               JNodes.nodesStatus[0]    = NLS_WIFI_AP;
               vTaskDelay(1000);
...
void httpSend(){
   ...
   uint32_t _ts = millis();
   for(int i=0; i<JHttp.q_msg.size(); i++ ){
      if( JHttp.q_msg[i] == "" )continue;
//   for( int i=0; i<2 && !JHttp.q_msg.empty(); i++){
      HTTPClient http;  
      j_servers[JHttp.q_ip[i]]["ms_send"] = _ts;
//      j_servers[JHttp.q_ip[i]]["ts_ok"]   = 0;
      String _msg   = JHttp.q_msg[i];
      Serial.printf("HTTP GET %s\n",_msg.c_str());        
      http.begin( _msg.c_str() );
      int httpCode = http.GET();
      if( httpCode == HTTP_CODE_OK ) {
         j_servers[JHttp.q_ip[i]]["ms_ok"] = _ts;
         Serial.println(http.getString());
         JHttp.q_msg[i] = "";
         JHttp.q_ip[i] = "";

//         JHttp.q_msg.erase(JHttp.q_msg.begin());
//         JHttp.q_ip.erase(JHttp.q_ip.begin());
      }
      else
         Serial.printf("Error HTTP %d\n",httpCode);
//         JHttp.q_msg.erase(JHttp.q_msg.begin());
      String s;
      serializeJson(j_servers, s);   
      Serial.print("!!!! JsonServers: ");
      Serial.println(s);
...
void taskButton1( void *pvParameters ){
   ...
   Serial.println(xPortGetCoreID());
#if defined(ARDUINO_WIFI_LoRa_32_V3)   
   btn1Semaphore    = xSemaphoreCreateBinary ();
#else   
   btn1Semaphore    = xSemaphoreCreateMutex();
   xSemaphoreTake( btn1Semaphore, 500 );
#endif
