...
extern NEOPIXEL_LED_STATUS_t nodesStatus[];
/*
class TSJsonNodes {
   private:
//      JsonArray j_cmd;
...
class TSJsonGates {
   public:
//      StaticJsonDocument<JSON_NODES_SIZE> Json;
      uint16_t countAll, countOnline, countFree, countBusy, countGate;
      uint16_t countFreeSave;
      bool changeCountFree;
      TSJsonGates();
      void set(String _id, uint16_t _all, uint16_t _online, uint16_t _free, uint16_t busy);
      void calculate();  
      void print();
};
...
class TSJsonHTTP {
   ...
   public:
      ...
      TSJsonHTTP(void);
      void setParams(String id,uint16_t cNodeAll,uint16_t cNodeOnline,uint16_t cNodeFree,uint16_t cNodeBusy);
...
//      void setMyAttribute(char *_name, char *_value);
      void pop(const char *_str);
};

//extern TSJsonNodes JNodes;
extern TSJsonHTTP  JHttp;
extern TSJsonGates JGates;
