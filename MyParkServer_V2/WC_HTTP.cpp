...
//   Pages.push_back({String("Сервера"),       String("/config_servers"), String("Список серверов"),         HTTP_handleConfigAdv1});
   ...
   Pages.push_back({String("Сброс"),        String("/reset"),      String("Сброс списка шлюзов"),           HTTP_handleReset});
   Pages.push_back({String("Тест ИК"),        String("/test"),      String("Тест отправки на табло через ИК"),HTTP_handleTest});
   Pages.push_back({String("API"),          String("/api"),         String("HTTP API"),                     HTTP_handleAPI});
...
void HTTP_printCSS(String &out){
    ...
    out += " table { background:#808080;color:#fbfbfb; }\n";
...
*/
 ...
 //  sprintf(s_tm,"%ld",time(nullptr));
   ...
   content += "  <tr>\n";
   content += "    <td>Шлюз</td>\n";
   content += "    <td>Паркомест</td>\n";
   content += "    <td>На связи</td>\n";
   content += "    <td>Свободно</td>\n";
   content += "    <td>Занято</td>\n";
//   content += "    <td>Время</td>\n";
   content += "  </tr>\n";
   ...
   for (JsonPair _item : _gates) {
       ...
       content += "  <tr>\n";
       content += "    <td>";
       content += _item.key().c_str();
       content += "</td>\n";
       content += "    <td>";
       content += String(_obj["countAll"].as<uint16_t>());
       content += "<t/d>\n";
       content += "    <td>";
       content += String(_obj["countOnline"].as<uint16_t>());
       content += "</td>\n";
       content += "    <td>";
       content += String(_obj["countFree"].as<uint16_t>());
       content += "</td>\n";
       content += "    <td>";
       content += String(_obj["countBusy"].as<uint16_t>());
       content += "</td>\n";
/*       
       char str[32];
       time_t t1 = (time_t)_obj["countBusy"].as<uint32_t>();
       struct tm *ti = localtime( &t1 ); 
       sprintf(str,"%02d.%02d.%02d %02d:%02d",ti->tm_mday,ti->tm_mon+1,ti->tm_year%100,ti->tm_hour,ti->tm_min);           
       content += "    <td>";
       content += str;
       content += "</td>\n";
 */
   ...
   content += "    <td>Всего</td>\n";
   content += "    <td>";
   content += String(JGates.countAll);
   content += "</td>\n";
   content += "    <td>";
   content += String(JGates.countOnline);
   content += "</td>\n";
   content += "    <td>";
   content += String(JGates.countFree);
   content += "</td>\n";
   content += "    <td>";
   content += String(JGates.countBusy);
   content += "</td>\n";
//   content += "    <td>&nbsp;</td>\n";
   content += "  </tr>\n";
   content += "<table>\n";
   gatesPrint("JsonGatesHttp");
/*
...
void HTTP_handleAPI(void) {
  ...
  String content = "";
  String _sid = "";
  uint16_t _all = 0, _online = 0, _free = 0, _busy = 0, _key = 0;
  bool _valid = true;
  
  if( webServer.hasArg("id")     )_sid    = webServer.arg("id");
  else _valid = false;
  if( webServer.hasArg("all")    )_all    = (uint16_t)webServer.arg("all").toInt();
  else _valid = false;
  if( webServer.hasArg("online") )_online = (uint16_t)webServer.arg("online").toInt();
  else _valid = false;
  if( webServer.hasArg("free")   )_free   = (uint16_t)webServer.arg("free").toInt();
  else _valid = false;
  if( webServer.hasArg("busy")   )_busy   = (uint16_t)webServer.arg("busy").toInt();
  else _valid = false;
  if( webServer.hasArg("key")    )_key    = (uint16_t)webServer.arg("key").toInt();
  else _valid = false;
/*
  if ( webServer.args()> 0 ){
...
*/  
  ...
  if( _valid ){
     uint16_t _key1 = JHttp.keyGen((char *)_sid.c_str(),_all,_online,_free,_busy);
     if( _key1 != _key ){
        _valid = false;
        content = "Bad crc key !!!";
        Serial.printf("???? HTTP API Error src %u %u\n",_key,_key1);
     }
  }
  else {
     content = "Params not found !!!";
     Serial.printf("???? Params not found\n");
  }
/*
  for( int i=0; i<webServer.args(); i++ ){
     Serial.print("Argnum=");
...
*/
 ...
 // int numP = numPages("API");
 // char str[20],add_str[64];
 ...
 // HTTP_printTail(content);
  if( _valid ){
     JGates.set(_sid, _all, _online, _free, _busy);
     JGates.calculate();
     JHttp.setParams(_sid,_all, _online, _free, _busy);
//     SendIR(JGates.countFree);     
     if( JGates.changeCountFree ){
//        saveNum = 0xffff;
//        SendIR(JGates.countFree);
        JGates.changeCountFree = false;
//        JHttp.setParams(strID,JGates.countAll,JGates.countOnline,JGates.countFree,JGates.countBusy);
        gatesSave();
     }
     content = "OK";     
  }
...
void HTTP_handleReset(void) {
  ...
  String content="";
  HTTP_printHeader(content,"Сброс",0);
  content += "Сброс выполнен.  Перезагрузка устройства через 3 сек\n";
  HTTP_printTail(content);
                   j_gates.clear();
                  JGates.calculate();
                  SendIR(JGates.countFree);
...
/*
 * Страница отправки ИК команды на табло
 */
void HTTP_handleTest(void) {
  ...
  if ( webServer.args()> 0 ){
     ...
     else if( webServer.hasArg("NUM_IR")     ){
         int n               = webServer.arg("NUM_IR").toInt();
         JGates.countFree = n;
         saveNum = 0xffff;
//         SendIR(n);
     }
  ...
  String content = "";
  int numP = numPages("Тест ИК");
  char str[20];
  HTTP_printHeader(content,numP,0);
  content += "  <form action='/test' method='GET'>\n";
  content += "   <fieldset>\n";
  content += "    <legend>Количество</legend>\n";
  content += "<div class=\"col1\"> Последняя программа:</div><div class=\"col2\">";
  content += String(JGates.countFree);
  content += "</div>\n";
  content += "   </fieldset>&nbsp;\n";
  char s[24];
  sprintf(s,"%u",JGates.countFree);
  content += "   <fieldset>\n";
  content += "    <legend>Отправить номер программы</legend>\n";

  HTTP_printInput(content,"Программа:","NUM_IR",s,16,32,HT_NUMBER);
  content += "   </fieldset>&nbsp;\n";
  content += "   <fieldset>\n";
  content +="<input type='submit' name='Send' value='Отправить'><br><br>"; 
//  content +="<input type='submit' name='Reset' value='Сбросить'>"; 
  content += "   </fieldset>\n";
